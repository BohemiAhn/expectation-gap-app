...
// Vercel KV에 저장/조회한다. 프론트(index.html)는 fetch('/api/state?...')로만 통신한다.
//
// 필요한 환경변수 (Vercel 프로젝트 > Storage > KV 연결 시 자동 생성됨):
//   KV_REST_API_URL
//   KV_REST_API_TOKEN
    ...
    if (req.method === 'GET') {
      ...
      if (type === 'expectation' && !role) {
        ...
        const [leader, member] = await Promise.all([
          kv.get(`expectation:${taskId}:leader`),
          kv.get(`expectation:${taskId}:member`),
        ]);
